...
function unpack(m::Mesh)
    ...
    for iedge = 1:m.nedge
        m.normalvec_edge[:, iedge] = (
            cos(m.angle_e[iedge]) * m.ulam_e[:, iedge] +
            sin(m.angle_e[iedge]) * m.uphi_e[:, iedge]
        )
        # tangvec is rotated counterclockwise by pi/2 so that (normalvec, tangvec, radial) is
        # always direct
        m.tangvec_edge[:, iedge] = (
            cos(m.angle_e[iedge] + pi / 2.0) * m.ulam_e[:, iedge] +
            sin(m.angle_e[iedge] + pi / 2.0) * m.uphi_e[:, iedge]
        )
    end
    ...
    for ix = 1:m.nx
        for iedge = 1:m.primal_deg[ix]
            edge_num = m.primal_edge[iedge, ix]
            m.unit_out[:, iedge, ix] =
                m.primal_ne[iedge, ix] * m.normalvec_edge[:, edge_num]
        end
    end
    ...
    for iedge = 1:m.nedge
        ...
        m.dual_le[iedge] =
            arclen(radialv(m.lat_i[c1], m.lon_i[c1]), radialv(m.lat_i[c2], m.lon_i[c2]))
    end
...
function overview(m::Mesh)::String
    ...
    s = s * "min - max of edge_left_right: " * string(min) * " - " * string(max) * "\n"

    max, index = findmax(m.edge_down_up)
    min, index = findmin(m.edge_down_up)
    s = s * "min - max of edge_down_up: " * string(min) * " - " * string(max) * "\n"

    max, index = findmax(m.primal_ne)
    min, index = findmin(m.primal_ne)
    s = s * "min - max of primal_ne: " * string(min) * " - " * string(max) * "\n"

    max, index = findmax(m.angle_e)
    min, index = findmin(m.angle_e)
    s = s * "min - max of angle_e: " * string(min) * " - " * string(max) * "\n"

    max, index = findmax(m.Riv2)
    min, index = findmin(m.Riv2)
    s = s * "min - max of Riv2: " * string(min) * " - " * string(max) * "\n"

    max, index = findmax(sum(m.Riv2, dims = 1))
    min, index = findmin(sum(m.Riv2, dims = 1))
    s = s * "min - max of Riv2 sum: " * string(min) * " - " * string(max) * "\n"

    s = s * "Check sphere area: " * string(1.0 / (4pi) * sum(m.Ai)) * "\n"

    s = s * "Check losange area: " * string(1.0 / (12pi) * sum(m.Ae)) * "\n"
...
function overview(m::MassField)::String
    ...
    s = s * "min - max of the field: " * string(minval) * " - " * string(maxval) * "\n"
...
function overview(m::DualField)::String
    ...
    s =
        s *
        "This is a dual field with ndual, nz: " *
        string(m.ndual) *
        " " *
        string(m.nz) *
        "\n"
...
function fill_flux(m::Mesh, fl::Flux, sf::DualField, factor = 1.0)
...
function overview(m::NormalVector)::String
    ...
    s =
        s *
        "This is a normal vector with nedge, nz: " *
        string(m.nedge) *
        " " *
        string(m.nz) *
        "\n"
...
function overview(m::TangentialVector)::String
    ...
    s =
        s *
        "This is a tangential vector with nedge, nz: " *
        string(m.nedge) *
        " " *
        string(m.nz) *
        "\n"
...
function overview(m::MassVector)::String
    ...
    s =
        s *
        "This is a mass vector field with nx, nz: " *
        string(m.nx) *
        " " *
        string(m.nz) *
        "\n"
    ...
    s =
        s *
        "min - max of the zonal component: " *
        string(minval) *
        " - " *
        string(maxval) *
        "\n"
    ...
    s =
        s *
        "min - max of the meridional component: " *
        string(minval) *
        " - " *
        string(maxval) *
        "\n"
...
function overview(m::MassVector3d)::String
    ...
    s =
        s *
        "This is a mass vector 3d field with nx, nz: " *
        string(m.nx) *
        " " *
        string(m.nz) *
        "\n"
    ...
    s =
        s *
        "min - max across the three components: " *
        string(minval) *
        " - " *
        string(maxval) *
        "\n"
...
function fill_field(m::Mesh, vector::MassVector3d, f::Function)
    ...
    for ix = 1:m.nx
        valu, valv = f(m.lat_i[ix], m.lon_i[ix])
        for iz = 1:vector.nz
            vector.F[:, ix, iz] = valu * m.ulam_i[:, ix] + valv * m.uphi_i[:, ix]
        end
    end
    return
end
...
function overview(m::EdgeVector)::String
    ...
    s =
        s *
        "This is an edge vector field with nedge, nz: " *
        string(m.nedge) *
        " " *
        string(m.nz) *
        "\n"
...
function primal2dual(m::Mesh, f::MassField, d::DualField)
    ...
    for iz = 1:f.nz
        for ix = 1:m.ndual
            d.F[ix, iz] = 0.0
            for iedge = 1:m.max_dual_deg
                wei = 1.0 - 2.0 * m.Riv2[iedge, ix]
...
function normal2tangential(m::Mesh, v::NormalVector, tang::TangentialVector)
    ...
    for iedge = 1:v.nedge
        for iz = 1:v.nz
            tang.F[iedge, iz] = sum(
                v.F[m.edge_kite[ind, iedge], iz] * m.edge_perp[ind, iedge] for ind = 1:4
            )
        end # loop on iz
    end # loop in iedge
    return
